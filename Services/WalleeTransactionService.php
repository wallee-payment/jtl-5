...
class WalleeTransactionService
{
     ...
     *      The email service.
     *
     * @see \Plugin\jtl_wallee\Services\WalleeMailService
     */
    ...
    public function __construct(ApiClient $apiClient, $plugin)
    {
        ...
        $this->mailService = new WalleeMailService($mailer, $mail, $db, $config);
    }
    ...
    public function confirmTransaction(Transaction $pendingTransaction): void
    {
        ...
        if ($createOrderAfterPayment === 1) {
            if ($this->isPreventFromDuplicatedOrders()) {
                [$orderNr, $orderReferenceNumber] = WalleeHelper::createOrderNo();
                ...
                [$orderNr, $orderReferenceNumber] = WalleeHelper::createOrderNo(false);
            }
            ...
            WalleeHelper::log("confirmTransaction: Order $orderId created. Setting cAbgeholt to NOT_SYNC_TO_WAWI ('" . self::NOT_SYNC_TO_WAWI . "').");
        ...
        try {
            ...
            if ($createOrderAfterPayment === 1 && !empty($orderId)) {
                ...
                $_SESSION['wlen_rollback_done_order_id'] = (int)$orderId;
            }
            $this->updateLocalWalleeTransaction((string)$transactionId, TransactionState::FAILED);
            throw $e;
        }
     ...
     * @return void
     */
    public function createLocalWalleeTransaction(string $transactionId, array $orderData, $state = TransactionState::PENDING): void
    {
     ...
     * @return void
     */
    public function updateLocalWalleeTransaction(string $transactionId, $state = null, $orderId = null): void
    {
        if ($state === null) {
    ...
    public function cancelOrderOnce(int $orderId): bool {
        ...
        if ((int)$order->cStatus === \BESTELLUNG_STATUS_STORNO) {
            WalleeHelper::log("cancelOrderOnce: Order $orderId is already cancelled.");
            return false;
        }

        if (empty($order->kZahlungsart) ) {
            WalleeHelper::log("cancelOrderOnce: Order $orderId has no payment method.");
            return false;
        }

        if (WalleeHelper::checkDBColumnExists('wallee_transactions', 'cancellation_email_sent')) {
            $db = Shop::Container()->getDB();
            ...
            if ($affected < 1) {
                ...
                if (!empty($row)) {
                    WalleeHelper::log("cancelOrderOnce: The cancellation email for order $orderId has already been marked as sent.");
                    return false;
                }
            }
        }
        ...
        WalleeHelper::log("cancelOrderOnce: Cancelling order $orderId.");
    ...
    public function restoreStock(int $orderId) {
        ...
        foreach ($order->Positionen as $pos) {
            if ((int)$pos->nPosTyp === \C_WARENKORBPOS_TYP_ARTIKEL && (int)$pos->kArtikel > 0) {
                WalleeHelper::log("cancelOrderOnce: Restoring stock for Product $pos->kArtikel (Qty: $pos->nAnzahl)");
    ...
    public function updateWawiSyncFlag(int $orderId, $flag) {
        WalleeHelper::log("updateWawiSyncFlag: Updating Order $orderId cAbgeholt to '$flag'.");
        Shop::Container()
            ->getDB()->update(
            'tbestellung',
            ['kBestellung',],
