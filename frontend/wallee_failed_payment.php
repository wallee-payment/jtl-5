...
WalleeHelper::log("failed_payment: User landed on failure page. TransactionId: " . ($transactionId ?? 'NONE'));
...
if ($transactionId) {
    ...
    if ($orderId > 0) {
        ...
        $alreadyRolledBack = (int)($_SESSION['wlen_rollback_done_order_id'] ?? 0) === $orderId;
        unset($_SESSION['wlen_rollback_done_order_id']);

        if ($alreadyRolledBack) {
            WalleeHelper::log("failed_payment: Order $orderId was already rolled back by confirmTransaction. Skipping cleanup.");
        } else {
            ...
            if (!empty($order->kZahlungsart)) {
                ...
                WalleeHelper::log("failed_payment: Cancelling Order $orderId via native cancelOrder routine.");
                ...
                foreach ($order->Positionen as $pos) {
                    if ((int)$pos->nPosTyp === \C_WARENKORBPOS_TYP_ARTIKEL && (int)$pos->kArtikel > 0) {
                        WalleeHelper::log("failed_payment: Restoring stock for Product $pos->kArtikel (Qty: $pos->nAnzahl)");
