...
{if $orders|@count > 0 && $orders}
    ...
    <form method="get" name="wallee-orders" action="{$currentUrl}">
        <div class="form-row">
            <!-- Label for the search input -->
            <label class="col-sm-auto col-form-label" for="orderSearch">{$translations.jtl_wallee_search}:</label>
    ...
    <form method="post" name="wallee-orders" action="{$postUrl}">
        {$jtl_token}
        <div class="table-responsive">
            <table class="table table-striped orders-table">
                <thead>
                <tr>
                    <th>{$translations.jtl_wallee_order_number}</th>
                    <th>{$translations.jtl_wallee_customer}</th>
                    <th>{$translations.jtl_wallee_payment_method}</th>
                    <th>{$translations.jtl_wallee_order_status}</th>
                    <th>{$translations.jtl_wallee_amount}</th>
                    <th></th>
                </tr>
                </thead>

                <div class="wallee-loader text-center" style="display: none">
...
{else}
    <div class="alert alert-info">
        ...
        {$translations.jtl_wallee_there_are_no_orders}
    </div>
{/if}
