<div>
    <div class="wallee-loader text-center">
        <img src="{$adminUrl}/assets/spinner.gif"/>
    </div>

    <div class="card">
        <div class="card-body">
            <p>{$translations.jtl_wallee_order_number}: {$orderNo}</p>
            <p>{$translations.jtl_wallee_transaction_id}: {$transactionId}</p>
            <p>{$translations.jtl_wallee_transaction_state}: <b>{$transactionState}</b></p>
        </div>
    </div>
    ...
    {if $transactionState != 'FULFILL' && $transactionState != 'AUTHORIZED' && $transactionState != 'PARTIALLY REFUNDED' && $transactionState != 'REFUNDED'}
        <div class="card">
            <div class="card-body">
                {$translations.jtl_wallee_transaction_no_possible_actions}
            </div>
        </div>
    {else}
        <div class="row">
            {if $transactionState == 'AUTHORIZED'}
                <div class="col-sm-3 col-xl-auto">
                    <button onclick="transactionAction({$transactionId}, 'complete')"
                        ...
                        {$translations.jtl_wallee_complete}
                    </button>
                </div>
                <div class="col-sm-3 col-xl-auto">
                    <button onclick="transactionAction({$transactionId}, 'cancel')"
                            ...
                            type="button">
                        {$translations.jtl_wallee_cancel}
                    </button>
                </div>
            {/if}
            {if $transactionState == 'FULFILL' || $transactionState == 'PARTIALLY REFUNDED' || $transactionState == 'REFUNDED'}
                <div class="col-sm-3 col-xl-auto">
                    <button class="refund btn btn-warning btn-block" data-toggle="collapse" data-target="#refund-form"
                            ...
                            type="button">
                        {$translations.jtl_wallee_refunds}
                    </button>
                </div>
                <div class="col-sm-3 col-xl-auto">
                    <a href="{$postUrl}&action=download_invoice&transactionId={$transactionId}" download
                       class="btn btn-outline-primary btn-block" type="button">
                        {$translations.jtl_wallee_download_invoice}
                    </a>
                </div>
                <div class="col-sm-3 col-xl-auto">
                    <a href="{$postUrl}&action=download_packaging_slip&transactionId={$transactionId}" download
                       class=" btn btn-outline-primary btn-block" type="button">
                        {$translations.jtl_wallee_download_packaging_slip}
                    </a>
                </div>
            {/if}
        </div>
...
<div class="collapse" id="refund-form">
    <div class="card card-body">
        {if $showRefundsForm}
            <div class="mt-5">
                <h2>{$translations.jtl_wallee_make_refund}</h2>
                <b>{$translations.jtl_wallee_amount_to_refund}:</b> <input type="text" id="refund-amount" value="{$amountToBeRefunded}"/>
                <button onclick="transactionAction({$transactionId}, 'refund', $('#refund-amount').val())" type="button"
                        class="btn btn-success">
                    {$translations.jtl_wallee_refund_now}
                </button>
            </div>
        {/if}

        <div class="mt-5">
            <h2>{$translations.jtl_wallee_refunded_amount} {$totalRefundsAmountText} / {$totalAmountText}</h2>
                ...
                <table class="table table-striped" id="refunds-details-table">
                    ...
                    <tr>
                        ...
                        <th class="text-right">
                            <div class="grid">
                                <div class="span12">
                                    {$translations.jtl_wallee_amount}:
                                </div>
                            </div>
                        </th>
                        <th class="text-right">
                            <div class="grid">
                                <div class="span12">
                                    {$translations.jtl_wallee_refund_date}
                                </div>
                            </div>
                        </th>
                    </tr>
                    ...
                    <tfooter>
                        <tr>
                            <th class="text-right">
                                <div class="grid">
                                    <div class="span12">{$translations.jtl_wallee_total}</div>
                                </div>
                            </th>
                            <th class="text-right">
                                <div class="grid">
            ...
            {else}
                <div class="card">
                    <div class="card-body">
                        {$translations.jtl_wallee_no_refunds_info_text}
                    </div>
                </div>
            {/if}
        </div>
...
<link rel="stylesheet" type="text/css" href="{$adminUrl}css/wallee-admin.css">
